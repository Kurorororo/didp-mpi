...
use crate::{node_communicator::TimeStampedNodeCommunicator, node_data_type::NodeDatatype};

pub struct HdLdBestFirstSearch<'a, T, N, M, L, R, B, F, V = Transition>
where
...
impl<'a, T, N, M, L, R, B, F, V> HdLdBestFirstSearch<'a, T, N, M, L, R, B, F, V>
where
    T: IsFloat + Ord + Display + Hash,
    <T as FromStr>::Err: Debug,
    CostToDump: From<T>,
    N: BfsNodeWithDistributedIdChain<T> + From<M> + NodeDatatype<T, S = M>,
    ...
    TransitionWithId<V>: Clone,
{
    ...
    pub fn new(
        ...
        hash_function: F,
        communicator: &'a SimpleCommunicator,
    ) -> HdLdBestFirstSearch<'a, T, N, M, L, R, B, F, V> {
    ...
    pub fn search(&mut self) -> (Solution<T, TransitionWithId<V>>, Vec<Statistics>) {
        ...
        let mut send_pool: BinaryHeap<(Rc<N>, Rank)> = BinaryHeap::new();
        ...
        loop {
            ...
            if let Some(node) = self.open.pop() {
                ...
                for (destination_rank, successor) in send_buffer.drain(..) {
                    if self.open.is_empty() {
                        ...
                        while send_pool.peek().map_or(false, |(peek, _)| peek.is_closed()) {
                            send_pool.pop();
                            self.closed_before_sent += 1;
                        }

                        if let Some(successor) = result.information {
                            send_pool.push((successor, destination_rank));
                        }
                    }
                }
            }
            ...
            if !send_pool.is_empty() {
                let open_peek = self.open.peek();

                while let Some((buffer_peek, destination_rank)) = send_pool.peek() {
                    ...
                    if open_peek.is_none() || buffer_peek >= open_peek.unwrap() {
                        self.node_communicator
                            .send(*destination_rank, buffer_peek.as_ref());
                        send_pool.pop();
                    } else {
                        break;
                    }
