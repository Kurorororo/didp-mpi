...
pub struct NodeDepthCommunicator<'a, C, M, T> {
    model: Rc<Model>,
    communicator: &'a C,
    tag: Tag,
    state_serializer: StateSerializer,
...
impl<'a, C, M, T> NodeDepthCommunicator<'a, C, M, T>
where
    C: Communicator,
    M: NodeDatatype<T, S = M>,
    T: Numeric + IsFloat,
{
    pub fn new(communicator: &'a C, tag: Tag, model: Rc<Model>) -> Self {
        ...
        let user_datatype = UserDatatype::structured(&blocklengths, &displacements, &types);
        let tmp_buffer = vec![0; offset + mem::size_of::<usize>()];
