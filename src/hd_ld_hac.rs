...
use mpi::{collective::SystemOperation, topology::SimpleCommunicator, traits::*, Rank, Tag};
...
pub struct HdLdHac<'a, T, N, M, L, R, B, F, V = Transition>
where
    ...
    V: TransitionInterface + Clone + Default,
{
    ...
    is_terminated: bool,
    closed_before_sent: usize,
}

impl<'a, T, N, M, L, R, B, F, V> HdLdHac<'a, T, N, M, L, R, B, F, V>
where
    T: Numeric + IsFloat + Ord + Display + Hash,
    <T as FromStr>::Err: Debug,
    CostToDump: From<T>,
    N: BfsNodeWithDistributedIdChain<T> + From<M> + Clone + NodeDatatype<T, S = M>,
    ...
    TransitionWithId<V>: Clone,
{
    ...
    fn finalize(
        ...
        if self.communicator.rank() == self.search.get_root_rank() {
            let mut closed_before_sent_sum = 0;
            root_process.reduce_into_root(
                &self.closed_before_sent,
                &mut closed_before_sent_sum,
                SystemOperation::sum(),
            );
            self.closed_before_sent = closed_before_sent_sum;
        } else {
            root_process.reduce_into(&self.closed_before_sent, SystemOperation::sum());
        }

        self.search.finalize(local_dual_bound)
    }

    pub fn search(&mut self) -> (Solution<T, TransitionWithId<V>>, Vec<Statistics>) {
        let mut keep_buffer = vec![];
        let mut send_buffer = vec![];
        let mut send_pool: BinaryHeap<(Rc<N>, usize, Rank)> = BinaryHeap::new();
        let mut send_registry = StateRegistry::new(self.model.clone());
        ...
        loop {
            ...
            if let Some((node, depth)) = self.pop_node_and_depth() {
                ...
                for (destination_rank, successor) in send_buffer.drain(..) {
                    if self.layered_open.len() <= self.current_depth
                        || self.layered_open[self.current_depth].is_empty()
                    {
                        self.node_communicator
                            .send(destination_rank, &successor, depth + 1);
                    } else {
                        let successor = N::from(successor);
                        let result = send_registry.insert(successor);
                        ...
                        while send_pool
                            .peek()
                            .map_or(false, |(peek, _, _)| peek.is_closed())
                        {
                            send_pool.pop();
                            self.closed_before_sent += 1;
                        }

                        if let Some(successor) = result.information {
                            send_pool.push((successor, depth + 1, destination_rank));
                        }
                    }
                }
            }

            if !send_pool.is_empty() {
                while let Some((buffer_peek, depth, destination_rank)) = send_pool.peek() {
                    if buffer_peek.is_closed() {
                        send_pool.pop();
                        self.closed_before_sent += 1;
                        continue;
                    }

                    let layered_open_peek = if self.layered_open.len() <= *depth {
                        None
                    } else {
                        self.layered_open[*depth].peek()
                    };

                    if layered_open_peek.is_none() || buffer_peek >= layered_open_peek.unwrap() {
                        self.node_communicator.send(
                            *destination_rank,
                            buffer_peek.as_ref(),
                            *depth,
                        );
        ...
        let (mut solution, statistics) = self.finalize(self.local_dual_bound);
    ...
    pub fn get_closed_before_sent(&self) -> usize {
        self.closed_before_sent
    }
