    ...
    TransitionWithId<V>: Clone,
{
    ...
    pub fn search(mut self) -> (Solution<T, TransitionWithId<V>>, Vec<Statistics>) {
        ...
        let open_best = self.open.peek().and_then(|node| node.bound(&self.model));
        let children_best = self
            .children
            .peek()
            .and_then(|node| node.bound(&self.model));
        let suspend_best = self.suspend.peek().and_then(|node| node.bound(&self.model));
